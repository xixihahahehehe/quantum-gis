...
void flowgraph::get_strength(vector<int>& v_strength)
{
    ...
    igraph_strength(&mygraph, &res,
        igraph_vss_all(), /*IGRAPH_IN  IGRAPH_OUT*/ IGRAPH_ALL,
        IGRAPH_NO_LOOPS, /*const igraph_vector_t *weights*/ &weights);
    ...
    for (int i = 0; i < vcount; i++)
    {
        int tmp = (int)VECTOR(res)[i];
        //cout << i << ": " << VECTOR(degree)[i] << endl;
        v_strength.push_back(tmp);
    }
